        ...
        <div className="container px-5 py-24 mx-auto flex flex-wrap flex-col">
          ...
          {Redux.state.ReceivedObject?.Retrieve?.ProjectSectionRetrieve?.projectGroups?.map((each, index) => (
            <React.Fragment key={index}>
              <div className={`flex flex-wrap text-center bg-[#603F83] dark:bg-[#C7D3D4] text-[#C7D3D4] dark:text-[#603F83] bg-opacity-60 px-4 py-6 rounded-lg ${activeProjectGroup === each.title ? "" : "hidden"}`}>
                          ...
                          <div className="flex items-center flex-wrap ">
                            ...
                            {each1?.links?.map((each2, index2) => (
                              ...
                              each2?.title === "Visit Admin" ? (
                                <span key={index2} className="mr-3 inline-flex items-center leading-none text-sm py-1 border-r-2 border-[#603F83] dark:border-[#C7D3D4]">
                                  <Link to={each2?.url} target="_blank" rel="noreferrer" >
